...
import {
    AspectRatio, Badge, Container, Flex, Heading, Image, Stack, Text, VStack
} from "@chakra-ui/react";

function Details(props) {
    const {id} = useParams()
    const fetchAnime = async () => axios.get(`https://api.jikan.moe/v4/anime/${id}`)
    ...
    return (<Container maxW="75%" height="100%">
        <Stack spacing='40px' height="100%" minH="100%">
            <Flex bgColor="gray.800" color="white" p="4" borderRadius="md" boxShadow="md" height="100%">
                ...
                <VStack align="left" spacing="4" borderColor="white" border="2px" p={6}>
                    ...
                    <Stack spacing="2">
                        {Array.isArray(data.data) && data.data.flatMap((x, i) => <Badge
                            key={i}><b>Tags: </b>{x.genres.name} </Badge>)}
